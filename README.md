...
系统安装后会自动在 config 目录中生成 plugins.php 的配置文件，
如果系统未生成可在命令行执行
...
```
或者在\config目录中新建`plugins.php`,内容为：
```php
<?php
return [
	// 是否自动读取取插件钩子配置信息
...
安装完成后访问系统时会在项目根目录生成名为`plugins`的目录，在该目录中创建需要的插件。
...
```php
<?php
namespace plugins\test;	// 注意命名空间规范
 ...
 */
class Plugin extends plugins	// 需继承think\plugins类
{
    // 该插件的基础信息
    public $info = [
        'name' => 'test',	// 插件标识
...
要访问插件中的controller时使用plugin_url生成url链接。
如下：
<a href="{:plugins_url('Action/link')}">link test</a>
或
<a href="{:plugins_url('test://Action/link')}">link test</a>
格式为：
